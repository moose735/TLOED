...
const SeasonMatchups = ({ season, leagueId, matchups, users, rosters, getDisplayTeamName }) => {
  ...
  useEffect(() => {
    ...
    if (Object.keys(matchups).length > 0) {
      ...
      // Only update if the selectedWeek is different to avoid unnecessary re-renders
      ...
      if (selectedWeek !== '') { // Only update if it's not already empty
        setSelectedWeek('');
  ...
  }, [season, matchups, selectedWeek]); // Added selectedWeek to dependency array to prevent stale closure for comparison
