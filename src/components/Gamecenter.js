                                        ...
                                        <div className="space-y-4 sm:space-y-6">
                                            ...
                                            {matchupRosterData ? (
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    {/* Starters comparison: span full width and show 3 columns (team1 | position | team2) */}
                                                    <div className="col-span-2">
                                                        <div className="bg-gray-50 rounded-lg p-3 sm:p-4 mobile-card">
                                                            <h5 className="font-semibold text-gray-700 mb-3 text-sm sm:text-base">Starting Lineups</h5>
                                                            <div className="space-y-0">
                                                                {matchupRosterData.team1.lineup.map((leftPlayer, idx) => {
                                                                    const rightPlayer = matchupRosterData.team2.lineup[idx] || {};
                                                                    ...
                                                                    const pos = leftPlayer.lineupPosition || rightPlayer.lineupPosition || '—';

                                                                    let outcomeLeft = 'pending';
                                                                    if (leftHasPlayed && rightHasPlayed) {
                                                                        const l = Number(leftPlayer.points ?? 0);
                                                                        const r = Number(rightPlayer.points ?? 0);
                                                                        outcomeLeft = l > r ? 'win' : l < r ? 'loss' : 'tie';
                                                                    }
                                                                    const outcomeRight = outcomeLeft === 'win' ? 'loss' : outcomeLeft === 'loss' ? 'win' : 'tie';

                                                                    return (
                                                                        <div key={`starter-row-${idx}`} className="grid grid-cols-3 items-center border-b border-gray-200 last:border-b-0 py-2">
                                                                            {/* Left: name, subline, outcome dot */}
                                                                            <div className="min-w-0">
                                                                                <div className="flex items-center gap-2">
                                                                                    <div className="font-medium text-gray-800 text-sm sm:text-base truncate">{leftPlayer.name}</div>
                                                                                    <div>{outcomeLeft === 'win' ? <span className="inline-block w-3 h-3 rounded-full bg-green-600" /> : outcomeLeft === 'loss' ? <span className="inline-block w-3 h-3 rounded-full bg-red-600" /> : <span className="inline-block w-3 h-3 rounded-full bg-gray-300" />}</div>
                                                                                </div>
                                                                                <div className="text-xs text-gray-400">{leftPlayer.position} · {leftPlayer.team || 'FA'}</div>
                                                                            </div>

                                                                            {/* Center: left score, position bubble, right score */}
                                                                            ...
                                                                            {/* Right: outcome dot, subline, name (right-aligned) */}
                                                                            <div className="min-w-0 text-right">
                                                                                <div className="flex items-center justify-end gap-2">
                                                                                    <div>{outcomeRight === 'win' ? <span className="inline-block w-3 h-3 rounded-full bg-green-600" /> : outcomeRight === 'loss' ? <span className="inline-block w-3 h-3 rounded-full bg-red-600" /> : <span className="inline-block w-3 h-3 rounded-full bg-gray-300" />}</div>
                                                                                    <div className="font-medium text-gray-800 text-sm sm:text-base truncate">{rightPlayer.name}</div>
                                                                                </div>
